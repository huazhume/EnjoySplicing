...
- (void)onPieceTouch:(ESPuzzleGamePiece *)piece {
    ...
    if (index == 0) {
        ...
        path = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"sound_%ld",rand] ofType:@"m4a"];
    }
    NSURL *url = [NSURL fileURLWithPath:path];
...
- (IBAction)onResetButton:(UIButton *)sender {
    ...
    NSString *path = [[NSBundle mainBundle]pathForResource:@"soso" ofType:@"m4a"];
    NSURL *url = [NSURL fileURLWithPath:path];
